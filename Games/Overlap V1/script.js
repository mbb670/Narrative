(()=>{
    ...
    function setAt(i){play.at=clamp(i,0,play.n-1);updatePlayUI();els.stage.focus()}
    ...
    function closeSuccess(){els.success.classList.remove('is-open');els.stage.focus()}
    ...
    function renderPreview(){
      const m=computed(puzzles[pIdx]);
      setCols(m.total);
      renderGrid(els.bGrid,m,false);els.bGrid.classList.add('showNums');
    ...
    els.stage.addEventListener('pointerdown',()=>els.stage.focus());
    ...
    els.success.addEventListener('click',(e)=>{if(e.target===els.success) closeSuccess()});
    els.sClose.addEventListener('click',closeSuccess);
    els.sAgain.addEventListener('click',resetPlay);
    els.sNext.addEventListener('click',()=>loadPuzzle(pIdx+1));
