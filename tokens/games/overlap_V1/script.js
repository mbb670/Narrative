...
els.pSave.addEventListener('click', () => {
  const m = computed(puzzles[pIdx]);
  if (!m.ok) {
    alert('Fix conflicts before saving.');
    return;
  }
  if (m.gaps?.length) {
    alert('Cover every column (no gaps) before saving.');
    return;
  }
  store.save();
  setDirty(false);
...
const norm = p => ({
  ...
  words: Array.isArray(p?.words) && p.words.length
    ? p.words.map(w => ({
        clue: String(w?.clue || ''),
