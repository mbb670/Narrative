// Style Dictionary v4 config + custom formatter for Matthew's collections → CSS
// Node >=18, ESM. SD exposes filePath + tokens to formats; we use those here.
// Docs: custom formats & helpers, outputReferences, filePath in filters. 
// (See: styledictionary.com/reference/hooks/formats, /utils, and Tokens Studio notes.)
...
const metaFromFile = (t) => {
  ...
  // matches ".../tokens/raw/<collection>/<set>.json"
...
StyleDictionary.registerFormat({
  ...
  formatter: ({ dictionary }) => {
    ...
    const isComposite =
      (t) => typeof t.value === "object" && t.value !== null && !Array.isArray(t.value);
    ...
    // 5–9. “Other” collections (alphabetical by collection; default first)
    ...
    // 10–11. Mode (light first, others alphabetical)
    ...
    if (styleTokens.length) {
      ...
      for (const t of styleTokens) {
        ...
        // generic composite class as fallback
      ...
      files: [
        {
          ...
          options: {
            // keep references as CSS vars whenever present
