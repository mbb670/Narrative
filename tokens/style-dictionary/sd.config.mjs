...
const collectStyleBlocks = (stylesJson) => {
  ...
  const walk = (node, p = []) => {
    ...
    if (
      ...
      if (type === "typography") {
        // .text-display-regular-lg { font-size: var(--…); ... }
      ...
      if (type === "boxShadow") {
        // .elevation-action { box-shadow: x y blur spread color; }
        const toShadow = (obj) =>
  ...
  format: () => {
    ...
    for (const col of topDirs) {
      ...
      for (const f of files) {
        ...
        const block = varsBlockFromFile(f).replace(
          /^:root\s+\{/,
          `[data-${col}="${setName}"] {`
        );
...
export default {
  source: ["tokens/raw/**/*.json"], // CLI requires a source; formatter reads raw files itself
  platforms: {
    css: {
      transformGroup: "css",
